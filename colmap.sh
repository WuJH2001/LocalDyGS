#  /amax/yjb/datasets/neural_3D/coffee_martini_down2_4dgs
...
do 
    value=$((i * 10))
    if [ $value -gt 299 ]; then
        value=299
    fi
    rm -rf $workdir/sparse_
    rm -rf $workdir/image_colmap
    python scripts/"$datatype"2colmap.py $workdir $value
    ...
    colmap stereo_fusion --workspace_path $workdir/colmap/dense/workspace --output_path $workdir/ply_files/fused_$value.ply
done

python scripts/downsample.py  $workdir/ply_files  $workdir/downsampled_points_10.ply
